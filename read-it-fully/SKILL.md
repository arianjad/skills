...
The failure mode is attention, not mechanics. Even when the bytes are already in context (pasted text, fully-loaded files), autoregressive generation biases the response toward salient anchors (start, headings, end) and away from the middle. Force intermediate generation steps that re-anchor attention across the whole input.
...
Don't auto-trigger on file size or perceived importance. If the user wants full reading, they'll say so. The skill biases toward under-triggering on purpose; over-triggering on every file drop would thrash.
...
The Read tool defaults to ~2000 lines per call. If the input is under that and not dominated by huge single lines, read it once. Chunking has a cost (extra tool calls, boundary-split risk) and buys nothing when the whole thing fits in one response.

The skill still fires for sub-threshold inputs. "Read fully" is about discipline (no skimming, cite specifics, walk the whole thing); it applies to a one-Read ingestion just as much as a chunked one.
...
- Cite locations when making claims: "section 4 says X", "line 220 shows Y", "page 7 discusses Z". Gives the user something to spot-check and matches the standing rule that reports are leads, not ground truth.
...
Plan: 8 chunks of 1000 lines each. Parallel Reads in one tool-use block, so the parent receives all 8 ranges before reasoning.
Execute: read all 8, synthesize.

**Academic paper (PDF, 40 pages)**
Survey: `pdf_info` → 40 pages; TOC shows 7 sections.
Plan: chunk by section. Serial, because Results depend on Methods.
